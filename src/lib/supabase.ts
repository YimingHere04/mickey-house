  ...
  async execute() {
    if (this.operation === 'select') {
      ...
      if (this.table === 'profiles' || this.table === 'users') {
        sourceList = [...mockUsers];
      } else if (this.table === 'bills') {
        sourceList = [...mockBills];
      } else if (this.table === 'payments') {
        sourceList = [...mockPayments];
      }
    ...
    if (this.operation === 'insert') {
      ...
      for (const record of recordsArray) {
        ...
        if (this.table === 'profiles' || this.table === 'users') {
          const exists = mockUsers.some(u => u.email.toLowerCase() === newRecord.email.toLowerCase());
          if (exists) {
            return { data: null, error: { message: 'A profile with this email already exists.' } };
          }
      ...
      if (this.table === 'profiles' || this.table === 'users') {
        mockUsers = mockUsers.map(user => {
          if (this.filterField && user[this.filterField as keyof UserProfile] === this.filterValue) {
            ...
            // If the user's ID changed, propagate to payments and set correct email
            if (changes.id && changes.id !== oldId) {
              mockPayments = mockPayments.map(p => {
                if (p.user_id === oldId) {
                  return { ...p, user_id: changes.id };
                }
                return p;
              });
        ...
        // Update currentUser if it matches the edited user
    ...
    if (this.operation === 'delete') {
      ...
      if (this.table === 'profiles' || this.table === 'users') {
        ...
        if (this.filterField) {
          mockUsers = mockUsers.filter(user => user[this.filterField as keyof UserProfile] !== this.filterValue);
        }
        ...
        if (this.filterField) {
          mockPayments = mockPayments.filter(pay => pay[this.filterField as keyof Payment] !== this.filterValue);
        }
